	...
	Drupal.behaviors.zeesalaamAdmin = {
		attach: function (context, settings) {
			...
			$(document).ready(function(){
				...
				$("#edit-field-publish-at-wrapper").hide();
			...
			$("input[name='field_publish_later']").once().change(function(){
               ...
               if(this.value == 0) {
                  ...
                  $("#edit-field-publish-at-wrapper").show();				  
			   } else {
			...
			$("#getvideo_data").once().on('click', function () {
				...
				var isSection = $.trim($('#edit-field-video-section option:selected').val());
				//alert(isSection);
				if(isSection == '_none' || isSection =='') {
					$('#edit-field-video-section option:selected').val('');
				}
				if(isYoutubeData == '_none' || isYoutubeData =='') {
					$('#edit-field-isyoutube').css('border-color', '#C80000');
					$('#edit-field-isyoutube option:selected').val('')
				    
				}else if(videourldata == '') {
					$('#edit-field-isyoutube').css('border-color', '');
					...
					var video_type = $.trim($('#edit-field-isyoutube option:selected').val());
					...
					if(video_type == 1){
						...
						var txtTranscodeAPIURL = tanscodeAPI + transcodeId;
							...
							$("#edit-field-english-title-0-value").val(result.title);
                            $("#edit-title-0-value").val(result.regional_title);
                            $("#edit-field-mobile-title-0-value").val(result.regional_title);							
							var sectionVal = result.section;
							
							if(sectionVal == 'Zee Salam') {
								var sectid = 4;
							} else {
								var sectid = 4;
							}
							$('#edit-field-video-section').val(sectid);
			...
			function getDuration(data1) {
				...
				$("#edit-field-mobile-title-0-value").val(data.items[0].snippet.title);
				
				var durationN  = data.items[0].contentDetails.duration;
				...
				$( "#edit-field-video-duration-0-value" ).val(durationN);
